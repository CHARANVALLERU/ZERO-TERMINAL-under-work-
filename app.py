...
# `st.session_state` is wiped on every full page reload, but the page
# auto-refreshes every NEWS_REFRESH_SECONDS — so we persist a minimal
# marker to a JSON file. The marker is the *only* thing the user clicks
# once; everything else (matrix regen, news, training) is background.
...
# ── Decide whether we need a fresh matrix this run ──────────────────────
#  * Hard requirements (no matrix, or crossed 4:00 PM boundary): first-time
#    splash path. The splash shows a loader; the calibration sleep is
#    trimmed so the user is through DIG & DIVE fast.
#  * Soft staleness (matrix older than 10 min after entry): silent
#    background regen — no splash, no sleep, no rerun, just a toast.
_should_full = False
if 'matrix' not in st.session_state or st.session_state.matrix is None:
    _should_full = True
else:
...
if _should_full:
    ...
    # Update unfulfilled feedback logs
    update_unfulfilled_feedback_logs()

    # Calibration removed: the matrix is computed instantly and the splash
    # was the only thing between the user and the main terminal. The
    # splash's own visual cycle (already in show_zero_digital_splash) is
    # enough to feel "alive" without burning wall-clock time.
    st.session_state.matrix = generate_prediction_matrix()

    # Log today's predictions as N/A for actuals
